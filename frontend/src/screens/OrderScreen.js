...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'FETCH_SUCCESS':
      return { ...state, loading: false, order: action.paylaod, error: '' };
    case 'FETCH_FAIL':
...
export default function OrderScreen() {
  ...
  const { id: orderId } = params; //rename
  ...
  function onApprove(data, actions) {
    return actions.order.capture().then(async function (details) {
      try {
        ...
        dispatch({ type: 'PAY_SUCCESS', paylaod: data });
        toast.success('Order is paid');
      } catch (err) {
        dispatch({ type: 'PAY_FAIL', paylaod: getError(err) });
